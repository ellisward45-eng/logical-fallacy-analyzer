...
<script>
    (() => {
        ...
        if (params.get("source") === "play") {
            sessionStorage.setItem("spotTheLiePlatform", "play");
        }

        const isPlayApp =
            sessionStorage.getItem("spotTheLiePlatform") === "play";
        ...
        document.addEventListener("DOMContentLoaded", () => {
            document.querySelectorAll("a[href]").forEach((link) => {
                ...
                url.searchParams.set("source", "play");
...
def stripe_webhook():
    ...
    if event["type"] == "checkout.session.completed":
        ...
        metadata = session_data["metadata"] if "metadata" in session_data else None

        if metadata and "customer_email" in metadata:
            customer_email = str(metadata["customer_email"]).strip().lower()
        elif "client_reference_id" in session_data and session_data["client_reference_id"]:
            customer_email = str(session_data["client_reference_id"]).strip().lower()
        else:
            customer_email = ""
        ...
        for item in analysis:
            ...
            for f in fallacies:
                if isinstance(f, str):
                    ...
                    label = (f.get("name") or f.get("label") or item.get("label") or "Unknown").strip()
                    confidence = int(
                        f.get("confidence")
                        or f.get("percent")
